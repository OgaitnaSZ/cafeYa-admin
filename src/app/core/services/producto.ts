...
export class ProductoService {
  ...
  subirFotoProducto(formData: FormData){
      ...
      this.http.post(`${this.apiUrl}producto/foto`, formData, { headers: this.tokenService.createAuthHeaders({ excludeContentType: true }) }).pipe(
          tap(() => {
  ...
  eliminarCategoria(categoria_id: number): void {
    ...
    this.http.delete(`${this.apiUrl}categoria/eliminar/${categoria_id}`, {
      headers: this.tokenService.createAuthHeaders(),
    }).pipe(
      tap(() => {
